...
<body>
	...
	<%-- 장단점, 급여, 근무환경, 관리자성향 리뷰수 ${parttimeeval.textlist.review_num} 리뷰점수 ${parttimeeval.textlist.review_score} --%>
...
<%-- <c:forEach items="${sessionScope.Ptevalall}" var="parttimeeval">
					<tr>
						<td class="cnt">${parttimeeval.proscons}</td>
						<td class="workcomnm"><a href="partTimeEvalView2.jsp">${parttimeeval.wage}</td>
						<td class="repcnt"><span class="icon"></span> <span class="text">${parttimeeval.environment}</span></td>
						<td class="repscore">${parttimeeval.incline}</td>
